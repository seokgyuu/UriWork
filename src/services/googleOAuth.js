...
class GoogleOAuthService {
  ...
  async linkWithFirebaseAuth(userInfo) {
    try {
      ...
      // OAuth 사용자 정보를 Firebase Auth와 연동
      // 실제 구현에서는 백엔드에서 Custom Token을 생성해야 함
      // 여기서는 기본적인 연동 로직만 구현
      
      const customToken = await this.createCustomToken(userInfo);
      if (customToken) {
        await signInWithCustomToken(auth, customToken);
        console.log('GoogleOAuth: Firebase Auth 연동 완료');
      }
