/**
 * 사용자 타입 선택 컴포넌트
 * 회원가입 시 사용자가 업체(사업자)인지 직원인지 선택하는 페이지
 * 선택한 타입에 따라 다른 가입 프로세스 진행
 * 업체는 캘린더 생성, 직원는 초대 코드 입력으로 구분
 */
             ...
             if (data.user_type) {
               ...
               if (isMounted) {
                 if (data.user_type === 'business') {
                   ...
                   console.log('UserTypeSelection: 직원 대시보드로 이동');
                   navigate('/worker');
                 }
               }
               return;
     ...
     try {
       ...
       while (retryCount < maxRetries) {
         try {
           ...
           // Firestore에서 사용자 타입 업데이트 (문서 없으면 생성)
           const userDocRef = doc(db, 'users', currentUser.uid);
                  ...
                  <div className="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center">
                    <svg className="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                 ...
                                 <div className="ml-4">
                   <h3 className="text-lg font-medium text-gray-900">직원</h3>
                   <p className="text-sm text-gray-500">
